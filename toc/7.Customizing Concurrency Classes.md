...
- 输出结果:  继承了`ThreadPoolExecutor`,然后重写了四个方法



ThreadPoolExecutor class and overriding four of its methods. The  beforeExecute()
...
methods to write statistics about the tasks executed in the executor to the console:
...
10 tasks to your executor that uses it and the other classes to demo their features.

Execute the program and you will see how the program shows the time span of each task that
is running and the statistics of the executor upon calling the  shutdown() method.
...
- 输出结果: new一个`PriorityBlockingQueue`当作`ThreadPoolExecutor`的构造参数,这样就可以自定义优先级线程池了


With the Executor framework, you only have to implement your tasks and send them to the
...
You have implemented the  MyPriorityTask class that implements the  Runnable
...
the rest of the tasks are executed based on their priority.


- 其他实现,比如`ScheduledThreadPoolExecutor`
...
classes, we use the factory instead of using the new operator.


- With this factory, we centralize the creation of objects gaining an advantage of easily
...
Java provides the  ThreadFactory interface to implement a  Thread object factory. Some
...
- 输出结果: 实现`ThreadFactory`,重写了`newThread()`方法
...
Once you had your own thread class, you have implemented a factory to create objects of
...
Thread object to execute that  Runnable object. In your case, you return a  MyThread object.


To check these two classes, you have implemented the  MyTask class that implements the
Runnable object. This is the task to be executed in threads managed by the  MyThread
object. A  MyTask instance puts its execution thread to sleep for 2 seconds.
...
- 输出结果: 和上面创建`ThreadFactory`的方式不一样,这个示例是用的是`Executors.newCachedThreadPool(threadFactory);`,threadFactory变量是自定义的`MyThreadFactory threadFactory=new MyThreadFactory("MyThreadFactory");`
...
- 输出结果: 和第4章学习的`ScheduledThreadPoolExecutor`类似,只不过这里继承了额它,实现了`decorateTask()` 和`scheduleAtFixedRate()`方法


In this recipe, you have implemented the  MyScheduledTask class to implement a custom
...
parameterized classes, with the type of data that will be returned by the tasks.
...
- 输出结果:  实现了`ForkJoinWorkerThreadFactory`,重写`newThread()`方法
...
- 输出结果: 创建一个抽象类,继承`ForkJoinTask<Void>`
...
- 输出结果: 继承`AbstractQueuedSynchronizer` 和`Lock`类,来自定义Lock
...
- 输出结果:  继承`AtomicInteger`类,来实现停车场功能,实现的非常优雅哦!
